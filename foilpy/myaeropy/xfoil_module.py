...
def call(airfoil, alfas=None, output='Cp', Reynolds=0, Mach=0, Ncrit=9,  # noqa C901
         plots=False, NACA=True, GDES=False, iteration=10, flap=None,
         PANE=False, NORM=True, direc=""):
    ...
    def submit(output, alfa):
        ...
        if output == "Alfa_L_0":
            ...
            if output == 'Cp':
                ...
                filename = file_name(airfoil, alfas, reynolds=Reynolds, Ncrit=Ncrit, output=output)
                try:
                    os.remove(filename)
                except OSError:
                    pass
    ...
    current_fpath = __file__
    xfoil_path = current_fpath.replace("xfoil_module.py", "") + "xfoil.exe"

    if posix:
        with open(os.devnull, 'w') as fp:
            # Calling xfoil with Popen
            # ps = sp.Popen(["/home/ss1870/Projects/HydrofoilAnalysis/pyfoil/myaeropy/xfoil.exe"],
            #             stdin=sp.PIPE,
            #             stdout=fp,
            #             stderr=None,
            #             encoding='utf8')
    ...
    if NORM is True:
        issueCmd('NORM')
    if NACA is False:
        issueCmd('load %s%s' % (direc,airfoil))
    else:
        issueCmd('%s' % airfoil)
    ...
    if output == 'Coordinates':
        ...
        if output == 'Polar' or output == 'Alfa_L_0':
            ...
            # filename functon.
            filename = file_name(airfoil, alfas, reynolds=Reynolds, Ncrit=Ncrit, output=output, direc="")
            #print(filename)
            try:
                os.remove(filename)
            except OSError:
...
def file_name(airfoil, alfas=None, reynolds=0, Ncrit=9, output='Cp', direc=""):
        ...
        if output == 'Polar':
            ...
            if type(alfas) == int or type(alfas) == float or \
               type(alfas) == np.float64 or type(alfas) == np.float32:
                ...
                alfa_i = alfas[0]
                alfa_f = alfas[-1]
            filename = '%s_%s_%s_%s_%s_%s' % (output, airfoil, reynolds, Ncrit, alfa_i, alfa_f)
...
def find_coefficients(airfoil, alpha, Reynolds=0, Ncrit=9, iteration=10,
    ...
    """
    filename = file_name(airfoil, alpha, reynolds=Reynolds, Ncrit=Ncrit, output='Polar')
    # If file already exists, there is no need to recalculate it.
    if not os.path.isfile(direc + filename):
        call(airfoil, alpha, Reynolds=Reynolds, Ncrit=Ncrit,
             output='Polar', iteration=iteration, NACA=NACA,
             PANE=PANE, GDES=GDES, direc=direc)

    coefficients = {}
    # Data from file
    Data = output_reader(direc + filename, output='Polar', delete=False)
    for key in Data:
        try:
